...
for (const backend of ['webgl', 'webgpu']) {
  test(
    'Heartfelt ' + backend + ' 雨滴可见、无外部素材且五个参数可恢复',
    async ({ page }, testInfo) => {
      ...
      await page.goto('./shaders/heartfelt/' + (backend === 'webgl' ? '?renderer=webgl' : ''));
      ...
      await testInfo.attach('rain-window-baseline', {
        body: initial.shot,
        contentType: 'image/png',
      });
      const sliders = page.locator('[data-parameter-list] input[type=range]');
      await expect(sliders).toHaveCount(5);
      for (const id of ['rain-speed', 'rain-amount', 'rain-fog', 'rain-refraction', 'rain-scale']) {
        ...
        // Bound sparse RGB rounding observed after mip LOD changes; never allow droplet movement.
        expect(difference.max).toBeLessThanOrEqual(2);
        expect(difference.mean).toBeLessThan(0.001);
      }
      await page.locator('#rain-refraction').evaluate((el: HTMLInputElement) => {
        el.value = '0';
        el.dispatchEvent(new Event('input'));
      });
