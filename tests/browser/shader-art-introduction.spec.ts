...
for (const backend of ['webgl', 'webgpu']) {
  test(
    'Shader Art Coding Introduction ' + backend + ' 递归光环可见、无外部素材且五个参数可恢复',
    async ({ page }, testInfo) => {
      ...
      await page.goto(
        './shaders/shader-art-introduction/' + (backend === 'webgl' ? '?renderer=webgl' : ''),
      );
      ...
      const inspect = async () => {
        ...
        const stats = await page.evaluate(async (png) => {
          ...
          // Exclude three device pixels at the screenshot boundary.
        ...
        expect(stats.chroma).toBeGreaterThan(10);
        expect(stats.max - stats.min).toBeGreaterThan(40);
        return { shot, stats };
      };
      const initial = await inspect();
      expect(initial.stats.mean).toBeGreaterThan(5);
      expect(initial.stats.white).toBeLessThan(0.35);
      await testInfo.attach('recursive-rings-baseline', {
        body: initial.shot,
        contentType: 'image/png',
      });
      const sliders = page.locator('[data-parameter-list] input[type=range]');
      await expect(sliders).toHaveCount(5);
      for (const id of ['art-speed', 'art-layers', 'art-tiling', 'art-frequency', 'art-glow']) {
        ...
        // WebGL2 occasionally rounds sparse channels by 1/255 after uniform changes.
        expect(difference.max).toBeLessThanOrEqual(1);
        expect(difference.mean).toBeLessThan(0.0001);
      }
